---
name: film-breakdown
description: 对电影、剧集、动漫或视频进行拉片分析。根据题材自动路由到对应框架，输出有判断的结构化逐段分析；对短视频/自媒体视频额外输出可复刻 SOP。用户说"拉片""分析这部电影""breakdown 这个视频""复刻这个博主""这部剧的镜头语言怎么样"时使用。
---
...
4. 短视频复刻：把对标视频拆成内容结构、技术实现和可执行 SOP
5. **HTML 报告生成（默认）**：所有分析的最终产物是一个自包含 HTML 文件，不是纯文本
...
在抽帧前先判断视频类型，并明确本次要拉哪些维度。不要所有视频都套同一张表。

常见类型：
- 口播教程类：重点拉台词结构、观点推进、字幕强调、图形动效、露脸/录屏比例
- 纯露脸口播：重点拉开头痛点、语速、表情手势、字幕节奏、punchline
- Tech/爆料/评测：重点拉参数顺序、证据画面、信源处理、购买建议、价格/风险收口
- Vlog/叙事类：重点拉事件结构、情绪曲线、生活细节、转场和音乐

维度示例：
- 内容层：选题、开头、段落结构、逐字稿、每 5 秒 punchline、CTA
- 技术层：拍摄、镜头布置、字幕样式、字体、图形特效、动效、剪辑节奏、调色、配乐、工具链
...
如果粗抽帧看不清某段动作、转场、字幕跳变或图形动效，对该区间追加密集抽帧（例如 0.1-0.3 秒一张），再分析。不要对整条视频盲目密集抽帧。
...
短视频拉片不是影评，目标是把对标视频拆成"内容结构 + 技术实现 + 复刻 SOP"。默认交付两份内容：

1. **拉片报告** -- 说明原作者怎么做
2. **复刻 SOP** -- 说明用户如果要复制，先做什么、再做什么

报告结构：
1. **封面** -- 标题、时长、段数/切点、题材、输入来源
2. **一句话结论** -- 这条视频到底靠什么成立，一句话说死
3. **本片拉片维度** -- 内容层 / 技术层分别拉哪些列
4. **可复刻度评分** -- 内容结构、素材获取、拍摄、图形/动效、剪辑节奏、工具链可得性
5. **关键发现** -- 3-5 条真正有用的规律，不要写空话
6. **配帧逐段表** -- 每段真实截帧 + 时间码 + 台词/字幕 + punchline + 图形/特效 + 作用
7. **图形层清单** -- 字体、颜色、字幕、动效、转场、素材类型
8. **工具链与确定性** -- 事实 / 高置信推断 / 低置信推断 / 未知，划清边界
9. **复刻 SOP** -- 可执行步骤，不写泛泛建议
...
| 时间码 | 精确到秒，有文件时必须写 |
| 代表帧 | 每段至少一张真实截帧 |
| 台词/字幕 | 有字幕/转写则引用；没有则引用画面可见字幕 |
| punchline | 本段最值得复刻的记忆点 |
| 图形/特效 | 字幕、贴纸、转场、动效、标注线、画中画等 |
| 作用 | 这一段为什么存在：钩子、立信、解释、反转、推进、收口 |
...
复刻 SOP 要按视频类型定制。比如录屏 + 露脸口播可写：口播稿 -> 录制 -> 粗剪 -> 图形动效 -> 合成 -> 导出。Tech 爆料类可写：选题 -> 信源/参数整理 -> 素材收集 -> 口播 -> 图形层 -> 剪辑 -> 价格/争议收口。
